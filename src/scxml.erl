...
reduce([], Acc) -> Acc;
reduce([#xmlElement{name = state, attributes = A, content = C} | T],
       #state{state = L} = Acc) ->
    Attrs = attr(A),
...
reduce([#xmlElement{name = transition, attributes = A, content = C} | T],
       #state{transition = L} = Acc) ->
    Attrs = attr(A),
...
reduce([#xmlElement{name = 'if', attributes = A, content = C} | T], Acc) ->
    ...
    E = #'if'{'cond' = parse(Cond), then = lists:reverse(reduce(L1, [])),
